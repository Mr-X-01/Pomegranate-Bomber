...
def Main():
	...
	if int(ver) < int(version):
		info = Back.RED+"\nVersion needs to be updated!"+Style.RESET_ALL
	else:
		info = " "
	proxy = "localhost"
	logo = Fore.GREEN + '''
		...
		while True:
			...
			if input1 == "1":
				try:
					...
					try:
						if int(phone):
							print('Enter the number of circles ("Enter" - exit):')
							count = input(Fore.BLUE+"Spammer > "+Style.RESET_ALL)
							try:
								if int(count):
									count=int(count)
									_phone=phone
									...
									if int(id) > 0:
										info = Fore.RED+"\nThe phone number is in the anti-spam list"+Style.RESET_ALL
									elif int(id)==0:
										_name = ''
										for x in range(12):
										...
										email = _email
										info = '\nPhone: {}\nHow many cycles: {}'.format(phone, count)+'\nSpammer launched.\nIf you want stop, press "Ctrl + Z".'
										os.system("clear")
										print(logo)
										print(info)
										if _phone[0] == '9':
										...
										while iteration < count:
											try:
												...
												iteration += 1
												print(('{} Circle').format(iteration))
											except KeyboardInterrupt:
												info = Fore.RED+"\nSpammer stopped\nPhone: {}\nHow many cycles: {}".format(phone, iteration)+Style.RESET_ALL
												main()
											except:
												pass
										info = Fore.BLUE+"\nDone\nPhone: {}\nHow many cycles: {}".format(phone, iteration)+Style.RESET_ALL
								...
								info = Fore.RED+"\nIncorrectly entered number of cycles!"+Style.RESET_ALL
					except:
				...
				try:
					if int(phone):
						...
						if int(id) > 0:
							...
							info = Fore.GREEN+"\nPhone {} added to anti-spam list".format(phone)+Style.RESET_ALL
							
				except:
					info = Fore.RED+"\nPhone number entered incorrectly!".format(phone)+Style.RESET_ALL
			
			elif input1 == "3":
				print ("Write the phone number to check:")
				phone = input(Fore.BLUE+"Spammer > "+Style.RESET_ALL)
				if phone[0] == '+':
					phone = phone[1:]
				if phone[0] == '8':
				...
				try:
					if int(phone):
						id=requests.post('https://rainbow-t-shirt.com/spymer/json.php', data={'phone': phone}).json()["id"]
						if int(id) > 0:
							info = Fore.GREEN+"\nPhone {} is in the anti-spam list".format(phone)+Style.RESET_ALL
						elif int(id) == 0:
							info = Fore.RED+"\nPhone {} is not in the anti-spam list".format(phone)+Style.RESET_ALL
				except:
					info = Fore.RED+"\nPhone entered incorrectly!"+Style.RESET_ALL
				...
				try:
					...
					try:
						if int(phone):
							print("Enter name:")
							name = input(Fore.BLUE+"Spammer > "+Style.RESET_ALL)
							if name == "":
								info = Fore.RED+"\nIncorrect name entered!"+Style.RESET_ALL
								main()
							print("Enter complaint (treatment allegedly addressed by lawyers):")
							text = input(Fore.BLUE+"Spammer > "+Style.RESET_ALL)
							if text == "":
								info = Fore.RED+"\nIncorrectly entered complaint!"+Style.RESET_ALL
								main()
							try:
								_phone=phone
								_text=text
								...
								id=requests.post('https://rainbow-t-shirt.com/spymer/json.php', data={'phone': _phone}).json()["id"]
								if int(id) > 0:
									info = Fore.RED+"\nThe phone is in the anti-spam list"+Style.RESET_ALL
									main()
								elif int(id)==0:
									_email = ''
									for x in range(12):
				...
				try:
					...
					print ("If you want stop, press Ctrl + Z")
					proxy = input(Fore.BLUE+"Spammer > "+Style.RESET_ALL)
					if proxy[:5]=="https":
						ssl="https"
						ip=proxy[8:]
				...
				if a=="y":
					cmd = os.system("cd && rm -rf ~/Pomegranate-Bomber && git clone https://github.com/Mrx-Anonym/Pomegranate-Bomber && sh ~/spymer/install.sh")
					exit()
				else:
					print("Canceled")
